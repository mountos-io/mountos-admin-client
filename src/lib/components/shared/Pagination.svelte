...
{#if totalPages > 1}
  <nav aria-label="Pagination" class="flex items-center justify-center gap-1 sm:gap-2 py-4">
    <span bind:this={prevRef}>
      <Button variant="outline" size="sm" class="min-h-[44px] sm:min-h-0" disabled={currentPage <= 1} onclick={() => onPageChange(currentPage - 1)} onkeydown={handlePrevKey} aria-label="Go to previous page">
        Previous
      </Button>
    </span>
    ...
    <span bind:this={nextRef}>
      <Button variant="outline" size="sm" class="min-h-[44px] sm:min-h-0" disabled={currentPage >= totalPages} onclick={() => onPageChange(currentPage + 1)} onkeydown={handleNextKey} aria-label="Go to next page">
        Next
      </Button>
    </span>
  </nav>
