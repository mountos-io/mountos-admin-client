...
<div class="space-y-4">
  <DiagramViewer ariaLabel="Object vs block storage diagram">
    <StorageTypeDiagram />
  </DiagramViewer>
  ...
  <ul class="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
    ...
    <li><span class="font-medium text-foreground">Block.</span> The client does block I/O against a block-volume facade (blockserv copysets, each active-active) that hides the same object store behind it and caches parts on a raw block device.</li>
  </ul>
</div>
