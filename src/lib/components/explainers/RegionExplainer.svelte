...
<div class="space-y-4">
  ...
  <ul class="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
    ...
    <li><span class="font-medium text-foreground">Cluster.</span> A volume-load partition inside a region. It runs the cluster-scoped services (dataserv, gcserv, optional gateways) and owns a slice of the region's volumes. Every cluster shares the region database and vault.</li>
