...
<div class="space-y-4">
  ...
  <ul class="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
    ...
    <li><span class="font-medium text-foreground">blockserv tolerates one instance failure.</span> With HA members writes still commit while one instance is down &mdash; the survivor falls back to the object-storage floor &mdash; and reads redirect to a sibling.</li>
