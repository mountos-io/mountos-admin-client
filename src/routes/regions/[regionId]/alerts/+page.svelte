<script lang="ts">
  ...
  import { TIME_RANGES, SEVERITY_LABELS, CATEGORIES } from '$lib/core/stores/alerts.svelte'
  ...
  const store = $derived(useRegionAlerts(regionId))
  const auth = useAuth()
...
<svelte:head><title>Region Alerts — mountOS Admin</title></svelte:head>

<div class="space-y-4">
  <div class="flex items-center gap-3">
    <Button variant="ghost" size="sm" onclick={() => goto(`/regions/${regionId}`)}>
      <ArrowLeft class="h-4 w-4" />
    </Button>
    <h1 class="text-2xl font-bold tracking-tight">Region Alerts</h1>
          ...
          {#each store.alerts as alert (alert.alertId)}
            ...
            <TableRow>
              ...
              <TableCell class="hidden xl:table-cell">
                {#if alert.nodeId}
                  <a href="/regions/{regionId}/{alert.nodeId}" class="font-mono text-primary hover:underline">{alert.nodeId}</a>
                {:else}
